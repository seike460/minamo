...
[![Node.js ≥24](https://img.shields.io/node/v/minamo)](package.json)
...
```ts
import { DynamoEventStore } from "minamo";
...
```ts
import { eventNamesOf, parseStreamRecord } from "minamo";
...
```ts
import { type CommandHandler, type InferSchemaOutput, executeCommand, validate } from "minamo";
...
- npm provenance 付き `minamo@0.1.0` publish
