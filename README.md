...
- **Future considerations** (v0.2.x, not v0.1.x): Aggregate-spanning `EventStoreTable` facade and a first-party `createCommandRunner` utility are tracked in [`docs/roadmap.md`](docs/roadmap.md). v0.1.x keeps the single-Aggregate `EventStore<TMap>` contract and the object-params `executeCommand` surface.
...
- [`docs/roadmap.md`](docs/roadmap.md) — Items deferred to v0.2.x and beyond
