...
4. 以下を手動で確認:
   - `npm view minamo@alpha` が 0.1.0-alpha.0 を返す
   - `npm view minamo@alpha dist` に `.integrity` と `signatures` が含まれる
...
- **npm Trusted Publishing の登録** (D-04): https://www.npmjs.com/settings/seike460/packages で `minamo` パッケージに対して GitHub Actions OIDC publisher を登録する (repo = `seike460/minamo` / workflow = `release.yml` / environment 任意)
- **repo 公開 + GitHub Pages 有効化** (D-06): private → public へ変更した上で Settings → Pages → Source: GitHub Actions

## Rollback

- npm 側: `npm deprecate minamo@<version> "broken release"` で installer に警告を表示
