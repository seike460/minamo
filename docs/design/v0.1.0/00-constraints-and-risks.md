...
- **緩和**: `RetryExhaustedError extends Error { cause: ConcurrencyError; attempts: number }` を定義 (ES2022 `cause` 準拠)
- **状態**: v0.1.0 では未導入（`executeCommand` は生の `ConcurrencyError` を throw）。**v0.2.0 で実装済み**（DEC-022 / concept.md §5.5）。`executeCommand` は retry 枯渇時に `RetryExhaustedError` を throw する
