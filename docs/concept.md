...
| メンテ状態（2026-05 時点） | 最終コミット 2025-10、core/adapter とも v2.4.2 (2025-04) | v3.0.0、最終 push 2026-05、アクティブ | — | 新規（v0.x） |

#### castore（`@castore/core`）

> **Fact:** castore の最新 `@castore/core` は v2.4.2（2025-04-18 公開）。DynamoDB adapter（`@castore/event-storage-adapter-dynamodb`）も同日 v2.4.2 が公開されており、core と歩調を合わせている。
> Source: https://www.npmjs.com/package/@castore/core (checked: 2026-05-30)
> Source: https://www.npmjs.com/package/@castore/event-storage-adapter-dynamodb (checked: 2026-05-30)

> **Fact:** GitHub リポジトリの最新コミットは 2025-10-12（Stars 274、未アーカイブ）。Issue #203 "State of this library?" が OPEN のまま。利用者からメンテ状態への懸念が表明されている。
> Source: https://github.com/castore-dev/castore/issues/203 (checked: 2026-05-30)
...
castore は TypeScript CQRS+ES ライブラリとして最も近い位置にある。generics を活用した型安全性、DynamoDB adapter、楽観的ロックを備える。core と DynamoDB adapter はともに v2.4.2（2025-04）で歩調を合わせており、Command 利用時の retry 設定も備えている。両者は同じ問題領域を扱うが、設計の重心が異なる:

1. **DynamoDB 専用設計 vs マルチ adapter** — castore は adapter パターンで複数ストレージに対応する汎用設計。minamo は DynamoDB を唯一の本番ストレージと定め、TransactWriteItems の制約や ConsistentRead を設計に直接組み込む。汎用性を捨てる代わりに DynamoDB 固有の最適化を得る
...
> **minamo が castore より劣る点:** castore は adapter パターンで DynamoDB 以外（PostgreSQL、MongoDB 等）にも対応可能。minamo は DynamoDB 専用であり、他の DB を使う場合は選択肢にならない。また castore はコミュニティが存在し（GitHub Stars 274、2026-05-30 時点。npm weekly downloads は minamo より多い）、実運用実績がある。minamo は新規プロジェクトであり、v0.x フェーズで実運用実績はまだない。

#### @ocoda/event-sourcing

> **Fact:** `@ocoda/event-sourcing` の最新バージョンは v3.0.0（2026-04-17 公開）。GitHub リポジトリの最新 push は 2026-05-28、Stars 265、アクティブにメンテナンスされている。
> Source: https://www.npmjs.com/package/@ocoda/event-sourcing (checked: 2026-05-30)

@ocoda/event-sourcing は NestJS フレームワーク上で CQRS+ES を実現するライブラリ。DynamoDB adapter を含み、アクティブにメンテナンスされている（v3.0.0、最終 push 2026-05-28）。ただし NestJS への依存が前提条件であり、フレームワーク非依存の minamo とはターゲットが異なる。

NestJS を使うチームにとっては DI・デコレータ・モジュールシステムとの統合が魅力。

> **Assumption:** @ocoda/event-sourcing の v3 系ドキュメントおよびソースコードに、楽観的ロック失敗時の自動リトライ機構への言及は確認できなかった（checked: 2026-05-30）。未調査の可能性があるため Assumption として扱う。
>
...
> **minamo が @ocoda より劣る点:** @ocoda は Snapshot、Saga、Event Upcasting を v3 系で提供しており、CQRS+ES の Read 側を含めた包括的なフレームワーク。minamo は Write 側のみを担い、Read 側は利用者に委ねる。NestJS エコシステムとの統合が必要な場合、minamo は選択肢にならない。
